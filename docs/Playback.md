...
`"sentence"` (utterances extracted with `segmentation: "sentence"`) and `"structure"` (`segmentation: "structure"`, the default) are synthesized by `speechNavigator` itself whenever the current utterance changes — real engines rarely emit a native sentence boundary mark. `detail.locate` is here a `LocatorOptions[]` (a `"word"` boundary's `detail.locate` is a single `LocatorOptions` — same key, the shape follows `detail.name`), already resolved for the active segmentation mode; see [Highlighting.md](Highlighting.md#highlighting-from-locateoffsets) for how to use it, and for the standalone `resolveUtteranceLocate()` function when driving playback without `speechNavigator`.
