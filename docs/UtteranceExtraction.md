...
- **`skip`** — drop a role and its subtree entirely (content + contextualization). `skippableRoles` export is the [roles.md skippable set](https://github.com/readium/guided-navigation/blob/main/roles.md#list-of-skippable-roles). Default: nothing skipped.
...
- **`contextualizations`** — override or extend the catalog for any subset of roles, at any depth (e.g. just `table.block.start`); merged over the locale's default. See "Contextualization catalog" below for its shape.
