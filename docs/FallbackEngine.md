...
Every `healthCheckIntervalMs`, the primary provider's `getVoices()` is called as a reachability probe (it naturally re-hits the network on every failed call, since nothing gets cached until it succeeds). Once it succeeds, the swap back happens at the next moment nothing is audibly playing, so a caller never hears a voice change mid-utterance — either the advance to the next utterance (its `speak(nextIndex)` call is intercepted and redirected to a freshly created primary engine started at that same index), an explicit `pause()` (the swap happens silently; the recovered primary only actually starts once `resume()` is called), or the whole queue finishing. It resumes with `rate`/`pitch`/`volume`/`setSpeakInContentLanguage` carried over and the originally requested voice restored. An `"enginerecovered"` event always fires before any playback from the recovered engine, with `detail: { voice }`.
