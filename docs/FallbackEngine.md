...
On a mid-playback swap, the new engine resumes at the same utterance — immediately, or only once `resume()` is called if the caller paused before the swap finished — with the best matching Web Speech voice for the failed voice's language, gender, and boundary-event support (each falling back to the wider match if none satisfies it — language-only if no matching-gender voice exists, then dropping the boundary requirement if no matching-gender voice also supports/lacks boundary events the same way), and `rate`/`pitch`/`volume`/`setSpeakInContentLanguage` carried over. A `"enginefallback"` event always fires before any playback from the new engine, with `detail: { reason, voice }`.
